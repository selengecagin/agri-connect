...
const ViewPostPage: React.FC = () => {
    ...
    const { post, user, comments, images } = data!;
    ...
    return (
        <div className="view-post-page">
            ...
            <div className="post-owner">
                <img src={`http://localhost:8080/api/images/fileSystem/id/${user.profilePhotoId}`} alt={user.name} className="rounded-full w-16 h-16"/>
                <p>{user.name}</p>
            </div>
            <h2>Comments</h2>
            {comments.map(comment => (
                <div key={comment.commentId} className="comment">
                    <img src={`http://localhost:8080/api/images/fileSystem/id/${comment.profilePhotoId}`} alt={comment.userId} className="rounded-full w-12 h-12"/>
                    <p>{comment.content}</p>
                </div>
            ))}
        </div>
